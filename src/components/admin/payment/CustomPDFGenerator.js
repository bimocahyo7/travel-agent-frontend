...
export async function generateCustomPDF(booking) {
  ...
  doc.text("TRAVEL AGENT TRIPNESIA", 14, 15);
  doc.setFontSize(10);
  doc.setFont(undefined, "normal");
  doc.text("Jl. Malang", 14, 20);
  doc.text("Telp: (021) 12345678 | Email: info@tripnesia.co.id", 14, 25);
  ...
  doc.text(
    `Jumlah Penumpang: ${booking.jumlah_penumpang ?? "-"}`,
    120,
    rightY + 14,
  );
  doc.text(
      ...
      [
        ...
        booking.payment?.amount
          ? new Intl.NumberFormat("id-ID", {
              ...
              Math.round(
                (booking.payment.amount || 0) / (booking.jumlah_penumpang || 1),
              ),
            )
          : "-",
        booking.payment?.amount
  ...
  doc.text(
    `Status Transaksi: ${booking.payment?.transaction?.status ?? "-"}`,
    120,
    afterTableY + 14,
  );
  ...
  doc.setTextColor(80);
  doc.text(
    "Terima kasih telah menggunakan layanan Tripnesia.",
    14,
    afterTableY + 28,
  );
  doc.setTextColor(120);
  doc.text("Struk ini sah tanpa tanda tangan.", 14, afterTableY + 34);

  // Footer
  doc.setFontSize(9);
  doc.setTextColor(180);
  doc.text(
    "Generated by Tripnesia System",
    105,
