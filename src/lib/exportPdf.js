...
export function generatePengajuanPdfDataUrl({ pengajuan, invoice }) {
  ...
  y += 6;
  doc.setFont('helvetica', 'normal');
  doc.text(`ID Pengajuan`, 20, y); doc.text(`: ${pengajuan?.id || '-'}`, 55, y);
  doc.text(`Instansi`, 110, y); doc.text(`: ${pengajuan?.institution || '-'}`, 140, y);
  y += 6;
  doc.text(`Pemohon`, 20, y); doc.text(`: ${pengajuan?.applicant || '-'}`, 55, y);
  doc.text(`Tujuan`, 110, y); doc.text(`: ${pengajuan?.destination || '-'}`, 140, y);
  y += 6;
  doc.text(`Tanggal Berangkat`, 20, y); doc.text(`: ${formatTanggalIndo(pengajuan?.departure_date)}`, 55, y);
  doc.text(`Tanggal Kembali`, 110, y); doc.text(`: ${formatTanggalIndo(pengajuan?.return_date)}`, 140, y);
  y += 6;
  doc.text(`Peserta`, 20, y); doc.text(`: ${pengajuan?.participants || '-'}`, 55, y);
  if (pengajuan?.status) { doc.text(`Status`, 110, y); doc.text(`: ${pengajuan.status}`, 140, y); }
  y += 10;
  ...
  const durasi = hitungDurasiHari(pengajuan?.departure_date, pengajuan?.return_date);
  const harga = invoice?.total !== undefined ? `Rp ${invoice.total.toLocaleString("id-ID", {minimumFractionDigits:2})}` : '-';
  ...
  doc.setFontSize(10);
  doc.setFont('helvetica', 'normal');
  doc.text("Terima kasih telah menggunakan layanan Travel Agent Indonesia.", 15, y);
  y += 5;
...
// Deprecated: use generatePengajuanPdfDataUrl and downloadPdfFromDataUrl for preview/download
